...
class AuthController extends Controller
{
    ...
    public function register(Request $request){
        ...
        $isEmail = filter_var($request->mobile_no,FILTER_VALIDATE_EMAIL)? true : false;

        if($isEmail==true){
            $request['email'] = $request->mobile_no;
        ...
        if($user->save()){
            //make login user
            Auth::attempt([filter_var($request->mobile_no,FILTER_VALIDATE_EMAIL) ? 'email' : 'mobile_no' => $request->mobile_no, 'password' => $request->password,'status'=>1]);
    ...
    public function login(Request $request){
        ...
        $isEmail = filter_var($request->mobile_no,FILTER_VALIDATE_EMAIL)? true : false;
        $this->validate($request,[
            'mobile_no'=>"required|".($isEmail==true ? 'email' : 'digits:11'),
            'name' => 'required',
            'password' => 'required|min:6',
        ]);


        if (Auth::attempt([filter_var($request->mobile_no,FILTER_VALIDATE_EMAIL) ? 'email' : 'mobile_no' => $request->mobile_no, 'password' => $request->password,'status'=>1])) {
          return response()->json([
              'status'=>'SUCCESS',
    ...
    public function resetCode(Request $request){
        ...
        if(empty($user)){
            ...
            (new SmsService())::SendUserPasswordResetCode($request->mobile_no,$code);
